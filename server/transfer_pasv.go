...
func (c *clientHandler) handlePASV() (err error) {
	...
	if c.transferTLS {
		tlsConfig, err2 := c.server.driver.GetTLSConfig()
		if err2 != nil {
			return c.writeMessage(StatusActionNotTaken, fmt.Sprintf("Cannot get a TLS config: %v", err2))
		}
